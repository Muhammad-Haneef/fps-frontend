"use client";

import { useId, useState } from "react";
import { cn } from "@/lib/utils";
import { useControllableState, useClickOutside } from "@/lib/hooks";
import FieldWrapper, { fieldBoxClasses } from "./FieldWrapper";
import FloatingPanel from "./FloatingPanel";

/**
 * YearPicker — pick a single year via a scrollable decade grid
 * (birth year, founding year, model year fields).
 */
export default function YearPicker({
  id,
  label,
  description,
  error,
  required,
  disabled,
  value,
  defaultValue = null,
  onChange,
  minYear = 1900,
  maxYear = new Date().getFullYear() + 10,
  size = "md",
  className,
}) {
  const autoId = useId();
  const fieldId = id || autoId;
  const [selected, setSelected] = useControllableState({ value, defaultValue, onChange });
  const [open, setOpen] = useState(false);
  const [decadeStart, setDecadeStart] = useState(Math.floor((selected || new Date().getFullYear()) / 10) * 10);
  const wrapperRef = useClickOutside(() => setOpen(false));

  const years = Array.from({ length: 12 }, (_, i) => decadeStart - 1 + i);

  return (
    <FieldWrapper id={fieldId} label={label} description={description} error={error} required={required} disabled={disabled} size={size} className={className}>
      <div className="relative" ref={wrapperRef}>
        <button
          type="button"
          id={fieldId}
          disabled={disabled}
          onClick={() => setOpen((o) => !o)}
          className={cn(fieldBoxClasses({ size, error, disabled }), "flex items-center text-left")}
        >
          <span className={cn(!selected && "text-neutral-400")}>{selected || "Select year"}</span>
        </button>

        {open && (
          <FloatingPanel anchorRef={wrapperRef} open={open} matchWidth={false} width={224} className="rounded-lg border border-neutral-200 bg-neutral-0 shadow-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <button type="button" onClick={() => setDecadeStart((d) => d - 10)} className="size-7 rounded-md hover:bg-neutral-100 flex items-center justify-center text-neutral-500">
                <ChevronIcon className="size-4 rotate-90" />
              </button>
              <span className="text-sm font-semibold">{decadeStart}s</span>
              <button type="button" onClick={() => setDecadeStart((d) => d + 10)} className="size-7 rounded-md hover:bg-neutral-100 flex items-center justify-center text-neutral-500">
                <ChevronIcon className="size-4 -rotate-90" />
              </button>
            </div>
            <div className="grid grid-cols-3 gap-1.5">
              {years.map((y) => {
                const disabledCell = y < minYear || y > maxYear;
                const outsideDecade = y < decadeStart || y >= decadeStart + 10;
                return (
                  <button
                    key={y}
                    type="button"
                    disabled={disabledCell}
                    onClick={() => {
                      setSelected(y);
                      setOpen(false);
                    }}
                    className={cn(
                      "h-10 rounded-md text-sm transition-colors tabular-nums",
                      selected === y ? "bg-brand-500 text-white" : "hover:bg-brand-50 text-neutral-700",
                      outsideDecade && "text-neutral-300",
                      disabledCell && "opacity-30 cursor-not-allowed hover:bg-transparent"
                    )}
                  >
                    {y}
                  </button>
                );
              })}
            </div>
          </FloatingPanel>
        )}
      </div>
    </FieldWrapper>
  );
}

function ChevronIcon({ className }) {
  return <svg viewBox="0 0 20 20" fill="currentColor" className={className}><path fillRule="evenodd" d="M5.23 7.21a.75.75 0 0 1 1.06.02L10 10.94l3.71-3.71a.75.75 0 1 1 1.06 1.06l-4.24 4.24a.75.75 0 0 1-1.06 0L5.21 8.29a.75.75 0 0 1 .02-1.08Z" clipRule="evenodd" /></svg>;
}
